...
for run in range(N_RUNS):
    ...
    if MODEL == "SVM_grid":
        ...
        try:
            ...
            pass
        #跑完訓練
        img2 = open_file("Datasets/leather2/leather2.mat")["leather2"]
        img2 = img2.astype(np.float32)

        test2_gt = open_file("Datasets/leather2/leather_gt2.mat")["leather_gt2"]
        # probabilities = test(model, img, hyperparams)
        probabilities = test(model, img2, hyperparams)
    ...
    #已算出結果
    mask = np.zeros(test2_gt.shape, dtype="bool")
    for l in IGNORED_LABELS:
        mask[test2_gt == l] = True
    prediction[mask] = 0
    ...
    display_predictions(
        color_prediction,
        viz,
        gt=convert_to_color(test2_gt),
        caption="Prediction2 vs. test2 ground truth",
    )
